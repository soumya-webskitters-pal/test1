'use strict';
//// smoke effect
const canvasBox = document.getElementById("canvas1");
if (typeof (canvasBox) != 'undefined' && canvasBox != null) {
...
`);
  ...
  window.addEventListener('DOMContentLoaded', (event) => {
    let set_color = document.querySelectorAll("[data-color]");
    for (let i = 0; i < set_color.length; i++) {
      set_color[i].parentNode.setAttribute("data-index", i);
      color_set.push(set_color[i].dataset.color.substring(1));
    }
    // console.log(color_set);
    currentColor = color_set[0];
  ...
  let all_color_list = document.querySelectorAll(".color_list li .liast_head");
  for (let i = 0; i < all_color_list.length; i++) {
    all_color_list[i].addEventListener("mouseenter", e => {
      all_color_list[i].parentNode.classList.add("active");
      currentColor = color_set[all_color_list[i].parentNode.dataset.index];
      let pointer = pointers[0];
      pointer.color = generateColor();
    });
    all_color_list[i].addEventListener("mouseout", e => {
      for (let j = 0; j < all_color_list.length; j++) {
        all_color_list[i].parentNode.classList.remove("active");
      }
    });
  }
  ...
  window.addEventListener('touchstart', e => {
    ...
    for (let i = 0; i < touches.length; i++) {
      console.log(touches);
      let posX = scaleByPixelRatio(touches[i].clientX) || scaleByPixelRatio(touches[i].pageX);
      let posY = scaleByPixelRatio(touches[i].clientY) || scaleByPixelRatio(touches[i].pageY);
